...
func (cm *CoSiModule) identifyConnection(conn net.Conn) error {
	...
	if !cm.valset.HasAddress(agtypes.GetAddress(pk, cm.chainID)) {
...
func (cm *CoSiModule) LeadCoSign(cosiAddr string, msg []byte) ([]byte, error) {
	...
	// all followers are synced with leader
	// pad the commitments
	for i := 0; i < cm.valset.Size()-len(cm.Commitments); i++ {
		cm.Commitments = append(cm.Commitments, cosi.Commitment{})
	}
	aggr := AggregateInfo{
